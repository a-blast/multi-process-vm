 ...
 * Sample Solution
 * 
 * Note that all methods expect the kernel page table to be enabled.
 * 
...
FrameAllocator::FrameAllocator(mem::MMU &memory_) 
: memory(memory_)
{
  // Set free list empty
  ...
  while (frame > 0) {
    memory.movb(frame, &free_list_head, sizeof(Addr));
...
bool FrameAllocator::AllocatePageFrames(uint32_t count, 
  ...
  if (count <= page_frames_free) {  // if enough to allocate
    ...
    while (count-- > 0) {
      ...
      memory.movb(&free_list_head, free_list_head, sizeof(uint32_t));
      --page_frames_free;
      
      // Clear page frame to all 0
      memory.movb(frame, zero_page.data(), kPageSize);
    }
...
bool FrameAllocator::FreePageFrames(uint32_t count,
                                     std::vector<uint32_t> &page_frames) {
  // Fetch free list info
  uint32_t page_frames_free = get_page_frames_free();
  uint32_t free_list_head = get_free_list_head();
...
std::string FrameAllocator::FreeListToString(void) const {
  ...
  while (next_free != kEndList) {
    ...
    memory.movb(&next_free, next_free, sizeof(uint32_t));
  }
  
  return out_string.str();
}

uint32_t FrameAllocator::get_page_frames_free() const {
  uint32_t page_frames_free;
  memory.movb(&page_frames_free, kPageFramesFree, sizeof(uint32_t));
  return page_frames_free;
}

uint32_t FrameAllocator::get_page_frames_total() const {
...
uint32_t FrameAllocator::get_free_list_head() const {
  ...
  memory.movb(&free_list_head, kFreeListHead, sizeof(uint32_t));
